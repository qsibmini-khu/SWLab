...
* LAST REVISED:  03/26/19 Doug Suh          C++ version
***************************************************************************/
#include <iostream>
#include <iomanip>
#include <fstream>
...
void update()
{
	...
	for (i = 1; i <= nsteps; i++) {                          // time t
		...
		for (j = 1; j <= tpoints; j++) {                     // location x
			/* global endpoints, boundary condition */
			if ((j == 1) || (j == tpoints))
				fxnp1[j] = 0.0;
			else
				do_math(j);
		...
		if (i == nsteps / 2) for (j = 1; j <= tpoints; j++) fxhalf[j-1] = fxn[j];  // to store
		if (i == nsteps / 4) for (j = 1; j <= tpoints; j++) fxforth[j-1] = fxn[j]; // to store
	}
}
...
int main(int argc, char *argv[])
{
	...
	fxhalf	= new float[tpoints];   // to store at t = 0.5 sec 
	fxforth = new float[tpoints];   // to store at t = 0.25 sec
